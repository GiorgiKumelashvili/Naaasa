<template>
    <v-card class="mx-auto" max-width="300">
        <v-card-text>
            ...
            <div>
                <v-card-subtitle
                    class="pa-0 py-1 text-body-1 grey--text text--darken-4"
                >
                    Media Type
                </v-card-subtitle>
            ...
            <div>
                <v-card-subtitle
                    class="pa-0 pt-5 pb-1 text-body-1 grey--text text--darken-4"
                >
                    Year Interval
                </v-card-subtitle>

                <v-range-slider
                    v-model="coreFilter.range"
                    :max="coreFilter.max"
                    :min="coreFilter.min"
                />
        ...
        <v-card-actions>
            ...
            <v-btn
                color="indigo"
                @click="$emit('cancelFilterAndFilter', final)"
                text
            >
                Filter
            </v-btn>
...
export default {
    ...
    methods: {
        setFinalMediaType: function(arr) {
            this.final.image = arr.includes(this.coreFilter.imageVal)
                ? true
                : false;
            this.final.video = arr.includes(this.coreFilter.videoVal)
                ? true
                : false;
        }
    ...
    watch: {
        "coreFilter.range": function() {
            this.final.year_start = this.coreFilter.range[0];
            this.final.year_end = this.coreFilter.range[1];
        }
    }
};
</script>

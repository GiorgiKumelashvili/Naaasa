<template>
	<div>
		<v-navigation-drawer v-model="sidebarParameters.drawer" :width="sidebarParameters.sidebarWidth" app>
			<!-- Sidebar Header -->
			<v-list-item class="pt-4">
				<v-list-item-content class="d-flex">
					<v-list-item-title class="title" v-text="sidebarParameters.title" />
				</v-list-item-content>

				<img src="@/assets/project.svg" width="30" height="30" alt="space exploration icon" />
			</v-list-item>
		...
		<v-app-bar app dense>
			<v-btn @click="toggle()" :class="{ flipped: !sidebarParameters.drawer }" icon>
			...
			<div class="d-flex align-center">
				<v-list-item>
					<v-badge bordered left top color="deep-purple accent-4" dot offset-x="20" offset-y="20">
						<v-btn icon>
						...
						<v-menu v-model="menu" :close-on-content-click="false" :nudge-width="200" offset-y>
							<template v-slot:activator="{ on, attrs }">
								<v-btn icon v-bind="attrs" v-on="on">
									...
									<v-avatar size="35" class="self-align-center">
							...
							<v-card>
								<v-list>
									<v-list-item>
										<v-list-item-avatar color="purple">
											<v-icon dark v-text="'mdi-account-cog-outline'" />
										</v-list-item-avatar>

										<v-list-item-content>
											<v-list-item-title v-text="'Mini Configuration'" />
										</v-list-item-content>
								...
								<v-list>
									...
									<v-list-item class="pa-0">
										<v-dialog v-model="sidebarParameters.logoutModelToggle" persistent>
											<template v-slot:activator="{ on, attrs }">
												<v-list-item key="logout-key" link>
													<v-list-item-icon>
														<v-icon>
															mdi-export
														</v-icon>
											...
											<v-card>
												...
												<v-card-actions>
													...
													<v-btn
														text
														@click="sidebarParameters.logoutModelToggle = false"
														v-text="'Disagree'"
													/>
													<v-btn text @click="logout" v-text="'Agree'" />
												</v-card-actions>
											</v-card>
										</v-dialog>
									</v-list-item>
...
import svg from '@/assets/project.svg'
...
export default {
	...
	computed: {
		moreThanMedium: function() {
			return this.sidebarParameters.activeSize === 'lg' || this.sidebarParameters.activeSize === 'xl'
		}
	},
	...
	watch: {
		...
		'$vuetify.breakpoint.name': {
			handler() {
				...
				this.sidebarParameters.drawer = screenSize === 'lg' || screenSize === 'xl' ? true : false
			},
			deep: true,
			immediate: true
		}

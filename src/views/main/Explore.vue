				...
				>
					<v-hover v-slot="{ hover }">
						...
						<v-img
							...
							class="pa-0 ma-0"
							:ref="el.data[0].title"
							@error="setErrorImage(el.data[0].title)"
							alt="empty img"
						>
	...
	methods: {
		...
		transport: function(obj) {
			this.$router.push({
				...
				params: {
					title: `/explore/${obj.data[0].title.replace(/\s/g, "")}`,
					data: obj
				}
			});
